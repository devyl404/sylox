...
def print_menu():
    ...
    print(f"{CYAN}4.{RESET} {YELLOW}TheFatRat{RESET}")
...
def run_tool(tool_name):
    ...
    if tool_name == "thefatrat":
        repo_dir = Path(__file__).resolve().parent / "dependencies" / "TheFatRat"
        if not repo_dir.exists():
            print(f"{RED}[!] Le dépôt TheFatRat n'a pas encore été installé.{RESET}")
            print(f"{YELLOW}Exécute d'abord install.py.{RESET}\n")
            input("Appuie sur Entrée pour revenir au menu...")
            return
        ...
        print(f"{BLUE}[>] Déplacement dans : {repo_dir}{RESET}")
        if os.geteuid() == 0:
            print(f"{CYAN}[>] Exécution : chmod +x setup.sh && ./setup.sh{RESET}\n")
            command = ["bash", "-lc", "chmod +x setup.sh && ./setup.sh"]
        else:
            print(f"{CYAN}[>] Exécution : sudo chmod +x setup.sh && sudo ./setup.sh{RESET}\n")
            command = ["sudo", "bash", "-lc", "chmod +x setup.sh && ./setup.sh"]
